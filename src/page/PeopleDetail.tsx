...
import { getDetailPeople } from 'services';
...
function PeopleDetail() {
  ...
  const {
    ...
    isError,
    error,
  } = useQuery<any, Error>(['detailPeople', personId], () => getDetailPeople(personId as string));
        ...
        <div className="flex -ml-8 -mr-8">
          ...
          <div className="flex-1 px-8 pt-[28.8px] pb-3">
            ...
            <h2 className="-mt-5 mb-[1.5em] leading-[1.25] text-2xl text-[#b5b5b5]">
              <span>
                {peopleDetail.original_name} (
                <Link
                  to={`/year/${new Date(peopleDetail.first_air_date).getFullYear()}`}
                  className="text-[#428bca] cursor-pointer hover:text-[#dcf836] transition-all duration-150"
                >
                  {new Date(peopleDetail.first_air_date).getFullYear()}
                </Link>
                )
              </span>
            </h2>
